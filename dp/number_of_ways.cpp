...
// Time Complexity: O(m * n * x) where m is number of faces,
//  n is number of dice and x is given sum.

// Auxiliary Space: O(n * x)
// We can add the following two conditions at the beginning of findWays() to improve 
// performance of the program for extreme cases (x is too high or x is too low) 
